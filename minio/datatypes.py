...
class ListPartsResult:
    ...
    @classmethod
    async def from_async_response(cls, response):
        return cls(await response.read())
...
ObjectVersionOwner = namedtuple("Owner", ("id", "display_name"))


class ObjectVersion:
    def __init__(self, key, last_modified, etag, size, owner, is_latest, version_id):
        self._key: str = key
        self._last_modified: datetime.datetime = last_modified
        self._etag: str = etag
        self._size: int = size
        self._owner: ObjectVersionOwner = owner
    ...
    def __str__(self):
        return """
        ObjectVersion(\n\tKey={}\n\tLast Modified={}\n\tETag={}\n\tSize={}\n\tOwner={}\n\tIsLatest={}\n\tVersion ID={})
        """.format(self._key, self._last_modified, self._etag, self._size, self._owner, self._is_latest,
                   self._version_id
                   )
    ...
    def from_xml(cls, element):
        return cls(
            ...
            findtext(element, "LastModified"),
            findtext(element, "ETag"),
            findtext(element, "Size"),
            ObjectVersionOwner(
                findtext(find(element, "Owner"), "ID"),
                findtext(find(element, "Owner"), "DisplayName")
            ),
            findtext(element, "IsLatest"),
            findtext(element, "VersionId")
        )


class ListVersionResult:
    def __init__(self, name, prefix, key_marker, next_version_id_marker, is_truncated, versions):
        self._name = name
        self._prefix = prefix
        self._key_marker = key_marker
        self._next_version_id_marker = next_version_id_marker
    ...
    def __str__(self):
        return """ListVersionResult(\n\tName={}\n\tPrefix={}\n\tIsTruncated={}\n\tVersions={}\n\t""".format(
            self._name, self._prefix, self._is_truncated, self._versions
        )
    ...
    def from_xml(cls, xml: str):
        ...
        return cls(
            findtext(element_tree, "Name"),
            findtext(element_tree, "Prefix"),
            findtext(element_tree, "KeyMarker"),
            findtext(element_tree, "NextVersionIdMarker"),
            findtext(element_tree, "IsTruncated"),
            list(map(ObjectVersion.from_xml, findall(element_tree, "Version")))
        )
