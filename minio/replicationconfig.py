...
class SourceSelectionCriteria:
    ...
    def from_xml(cls, element):
        ...
        return cls(
            ...
            else SseKmsEncryptedObjects.from_xml(element)
        )
...
class Metrics:
    ...
    def from_xml(cls, element):
        ...
        event_threshold = EventThreshold.from_xml(element)
        status = findtext(element, "Status", True)
...
class Destination:
    ...
    def from_xml(cls, element):
        ...
        access_control_translation = (
            ...
            else AccessControlTranslation.from_xml(element)
        )
        ...
        encryption_config = (
            ...
            else EncryptionConfig.from_xml(element)
        )
        metrics = (
            None if find(element, "Metrics") is None
            else Metrics.from_xml(element)
        )
        replication_time = (
            None if find(element, "ReplicationTime") is None
            else ReplicationTime.from_xml(element)
        )
...
class Rule(BaseRule):
    ...
    def from_xml(cls, element):
        ...
        delete_marker_replication = (
            ...
            else DeleteMarkerReplication.from_xml(element)
        )
        destination = Destination.from_xml(element)
        existing_object_replication = (
            None if find(element, "ExistingObjectReplication") is None
            else ExistingObjectReplication.from_xml(element)
        )
        ...
        source_selection_criteria = (
            ...
            else SourceSelectionCriteria.from_xml(element)
        )
        status = findtext(element, "Status", True)
...
class ReplicationConfig:
    ...
    def from_xml(cls, element):
        ...
        rules = []
        for tag in elements:
            rules.append(Rule.from_xml(tag))
