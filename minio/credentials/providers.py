...
async def _urlopen(http_client, method, url, body=None, headers=None):
    """Wrapper of urlopen() handles HTTP status code."""
    res = await http_client.request(method, url, data=body, headers=headers)
...
class AssumeRoleProvider(Provider):
    ...
    async def retrieve(self):
        ...
        self._credentials = _parse_credentials(
            await res.text(), "AssumeRoleResult",
        )
...
def _check_loopback_host(url):
    ...
    # FIXME: Remove urllib3 dependency
...
def _get_jwt_token(token_file):
    ...
    try:
        # FIXME: Convert to async functions
...
class IamAwsProvider(Provider):
    ...
    async def fetch(self, url):
        ...
        # FIXME: Convert into one call
        res = await _urlopen(self._http_client, "GET", url)
        data = await res.json()
    ...
    async def retrieve(self):
        ...
        self._credentials = await self.fetch(url)
        return self._credentials
...
class LdapIdentityProvider(Provider):
    ...
    async def retrieve(self):
        ...
        self._credentials = _parse_credentials(
            await res.text(), "AssumeRoleWithLDAPIdentityResult",
        )
...
class WebIdentityClientGrantsProvider(Provider):
    ...
    async def retrieve(self):
        ...
        res = await _urlopen(self._http_client, "POST", url)
