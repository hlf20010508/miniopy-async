...
class CommonConfig:
    ...
    def parsexml(element):
        ...
        if element is not None:
            ...
            for tag in elements:
                filter_rule = FilterRule.from_xml(tag)
...
class NotificationConfig:
    ...
    def from_xml(cls, element):
        ...
        for tag in elements:
            cloud_func_config_list.append(CloudFuncConfig.from_xml(tag))
        elements = findall(element, "QueueConfiguration")
        queue_config_list = []
        for tag in elements:
            queue_config_list.append(QueueConfig.from_xml(tag))
        elements = findall(element, "TopicConfiguration")
        topic_config_list = []
        for tag in elements:
            topic_config_list.append(TopicConfig.from_xml(tag))
        return cls(
