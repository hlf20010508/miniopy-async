...
class DecryptReader:
    ...
    def __init__(self, response: ClientResponse, header: bytes, secret: bytes):
    ...
    @classmethod
    async def init_async(cls, response: ClientResponse, secret: bytes):
        """Initialize DecryptReader asynchronously."""
        header = await response.content.read(41)
        return cls(response, header, secret)
    ...
    async def _read_chunk(self) -> bool:
        ...
        while len(self._chunk) != (1 + _MAX_CHUNK_SIZE):
            chunk = await self._response.content.read(
                1 + _MAX_CHUNK_SIZE - len(self._chunk)
            )
            self._chunk += chunk
            if len(chunk) == 0:
...
async def decrypt(response: ClientResponse, secret_key: str) -> bytes:
    """Decrypt response data."""
    result = b""
    with await DecryptReader.init_async(response, secret_key.encode()) as reader:
