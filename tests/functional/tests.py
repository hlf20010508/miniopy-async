...
async def _gen_bucket_name():
    """Generate random bucket name."""
    return "minio-py-test-{0}".format(uuid4())


async def _get_sha256sum(filename):
...
async def _get_random_string(size):
...
class LimitedRandomReader:  # pylint: disable=too-few-public-methods
    ...
    async def __init__(self, limit):
        self._limit = limit

    async def read(self, size=64*KB):
...
async def _call(log_entry, func, *args, **kwargs):
...
async def _call_test(func, *args, **kwargs):
...
async def test_make_bucket_default_region(log_entry):
...
async def test_make_bucket_with_region(log_entry):
...
async def test_negative_make_bucket_invalid_name(  # pylint: disable=invalid-name
        log_entry):
...
async def test_list_buckets(log_entry):
    """Test list_buckets()."""
...
async def test_select_object_content(log_entry):
    ...
    try:
        await _CLIENT.make_bucket(bucket_name)
        content = io.BytesIO(b"col1,col2,col3\none,two,three\nX,Y,Z\n")
        await _CLIENT.put_object(bucket_name, csvfile, content,
        ...
        data = await _CLIENT.select_object_content(bucket_name, csvfile, request)
        # Get the records
        records = io.BytesIO()
        ...
        await _CLIENT.remove_object(bucket_name, csvfile)
        await _CLIENT.remove_bucket(bucket_name)


async def _test_fput_object(bucket_name, object_name, filename, metadata, sse):
    """Test fput_object()."""
    try:
        await _CLIENT.make_bucket(bucket_name)
        if _IS_AWS:
            await _CLIENT.fput_object(bucket_name, object_name, filename,
                                metadata=metadata, sse=sse)
        else:
            await _CLIENT.fput_object(bucket_name, object_name, filename, sse=sse)

        await _CLIENT.stat_object(bucket_name, object_name, ssec=sse)
    finally:
        await _CLIENT.remove_object(bucket_name, object_name)
        await _CLIENT.remove_bucket(bucket_name)


async def test_fput_object_small_file(log_entry, sse=None):
...
async def test_fput_object_large_file(log_entry, sse=None):
...
async def test_fput_object_with_content_type(  # pylint: disable=invalid-name
        log_entry):
...
async def _validate_stat(st_obj, expected_size, expected_meta, version_id=None):
...
async def test_copy_object_no_copy_condition(  # pylint: disable=invalid-name
    ...
    try:
        ...
        await _CLIENT.put_object(bucket_name, object_source, reader, size, sse=ssec)
        await _CLIENT.copy_object(
            bucket_name, object_copy, sse=ssec,
            source=CopySource(bucket_name, object_source, ssec=ssec_copy),
        )
        st_obj = await _CLIENT.stat_object(bucket_name, object_copy, ssec=ssec)
        _validate_stat(st_obj, size, {})
    finally:
        await _CLIENT.remove_object(bucket_name, object_source)
        await _CLIENT.remove_object(bucket_name, object_copy)
        await _CLIENT.remove_bucket(bucket_name)


async def test_copy_object_with_metadata(log_entry):
    ...
    try:
        ...
        size = 1 * KB
        reader = LimitedRandomReader(size)
        await _CLIENT.put_object(bucket_name, object_source, reader, size)
        ...
        # Verification
        st_obj = await _CLIENT.stat_object(bucket_name, object_copy)
...
async def test_copy_object_etag_match(log_entry):
    ...
    try:
        ...
        # Verification
        source_etag = await _CLIENT.stat_object(bucket_name, object_source).etag
...
async def test_copy_object_negative_etag_match(  # pylint: disable=invalid-name
        log_entry):
...
async def test_copy_object_modified_since(log_entry):
...
async def test_copy_object_unmodified_since(  # pylint: disable=invalid-name
        log_entry):
...
async def test_put_object(log_entry, sse=None):
    """Test put_object()."""
    ...
    try:
        ...
        await _CLIENT.put_object(bucket_name, object_name, reader, length, sse=sse)
        await _CLIENT.stat_object(bucket_name, object_name, ssec=sse)
        ...
        await _CLIENT.put_object(bucket_name, object_name + "-metadata", reader,
        ...
        # metadata.
        st_obj = await _CLIENT.stat_object(bucket_name, object_name + "-metadata",
                                     ssec=sse)
        normalized_meta = {
        ...
        await _CLIENT.remove_object(bucket_name, object_name)
        await _CLIENT.remove_object(bucket_name, object_name+'-metadata')
        await _CLIENT.remove_bucket(bucket_name)


async def test_negative_put_object_with_path_segment(  # pylint: disable=invalid-name
        log_entry):
    ...
    try:
        await _CLIENT.make_bucket(bucket_name)
        await _CLIENT.put_object(bucket_name, object_name, io.BytesIO(b''), 0)
        await _CLIENT.remove_object(bucket_name, object_name)
...
async def _test_stat_object(log_entry, sse=None, version_check=False):
    """Test stat_object()."""
    ...
    try:
        ...
        # Fetch saved stat metadata on a previously uploaded object with
        # metadata.
        st_obj = await _CLIENT.stat_object(
        ...
        await _CLIENT.remove_object(bucket_name, object_name, version_id=version_id1)
        await _CLIENT.remove_object(
            bucket_name, object_name+'-metadata', version_id=version_id2,
        )
        await _CLIENT.remove_bucket(bucket_name)


async def test_stat_object(log_entry, sse=None):
    """Test stat_object()."""
    _test_stat_object(log_entry, sse)


async def test_stat_object_version(log_entry, sse=None):
    """Test stat_object() of versioned object."""
    _test_stat_object(log_entry, sse, version_check=True)


async def _test_remove_object(log_entry, version_check=False):
    """Test remove_object()."""
...
async def test_remove_object(log_entry):
    """Test remove_object()."""
    _test_remove_object(log_entry)


async def test_remove_object_version(log_entry):
    """Test remove_object() of versioned object."""
    _test_remove_object(log_entry, version_check=True)


async def _test_get_object(log_entry, sse=None, version_check=False):
    """Test get_object()."""
    ...
    try:
        ...
        object_data = await _CLIENT.get_object(
        ...
        await _CLIENT.remove_object(bucket_name, object_name, version_id=version_id)
        await _CLIENT.remove_bucket(bucket_name)


async def test_get_object(log_entry, sse=None):
    """Test get_object()."""
    _test_get_object(log_entry, sse)


async def test_get_object_version(log_entry, sse=None):
    """Test get_object() for versioned object."""
    _test_get_object(log_entry, sse, version_check=True)


async def _test_fget_object(log_entry, sse=None, version_check=False):
    """Test fget_object()."""
...
async def test_fget_object(log_entry, sse=None):
    """Test fget_object()."""
    _test_fget_object(log_entry, sse)


async def test_fget_object_version(log_entry, sse=None):
    """Test fget_object() of versioned object."""
    _test_fget_object(log_entry, sse, version_check=True)


async def test_get_object_with_default_length(  # pylint: disable=invalid-name
        log_entry, sse=None):
    ...
    await _CLIENT.make_bucket(bucket_name)
    try:
        await _CLIENT.put_object(bucket_name, object_name,
                           LimitedRandomReader(size), size, sse=sse)
        # Get half of the object
        object_data = await _CLIENT.get_object(bucket_name, object_name,
...
async def test_get_partial_object(log_entry, sse=None):
...
async def _test_list_objects(log_entry, use_api_v1=False, version_check=False):
    """Test list_objects()."""
    ...
    try:
        ...
        objects = await _CLIENT.list_objects(
...
async def test_list_objects_with_prefix(log_entry):
    ...
    try:
        ...
        for i in range(no_of_created_files):
            await _CLIENT.put_object(bucket_name,
                               "{0}{1}_{2}".format(
                                   path_prefix,
                                   i,
                                   object_name,
...
async def test_list_objects_with_1001_files(  # pylint: disable=invalid-name
        log_entry):
    ...
    try:
        ...
        for i in range(no_of_created_files):
            await _CLIENT.remove_object(bucket_name,
                                  "{0}_{1}".format(object_name, i))
        await _CLIENT.remove_bucket(bucket_name)


async def test_list_objects(log_entry):
    """Test list_objects()."""
    _test_list_objects(log_entry)


async def test_list_object_versions(log_entry):
    """Test list_objects() of versioned object."""
    _test_list_objects(log_entry, version_check=True)


async def test_presigned_get_object_default_expiry(  # pylint: disable=invalid-name
        log_entry):
    ...
    try:
        size = 1 * KB
        await _CLIENT.put_object(bucket_name, object_name, LimitedRandomReader(size),
                           size)
        presigned_get_object_url = await _CLIENT.presigned_get_object(
...
async def test_presigned_get_object_expiry(  # pylint: disable=invalid-name
        log_entry):
...
async def test_presigned_get_object_response_headers(  # pylint: disable=invalid-name
        log_entry):
...
async def test_presigned_get_object_version(  # pylint: disable=invalid-name
        log_entry):
    ...
    version_id = None
    try:
        await _CLIENT.set_bucket_versioning(bucket_name, VersioningConfig(ENABLED))
        size = 1 * KB
        result = await _CLIENT.put_object(
...
async def test_presigned_put_object_default_expiry(  # pylint: disable=invalid-name
        log_entry):
    ...
    await _CLIENT.make_bucket(bucket_name)
    try:
        presigned_put_object_url = await _CLIENT.presigned_put_object(
            bucket_name, object_name)
        ...
        await _CLIENT.stat_object(bucket_name, object_name)
    finally:
        await _CLIENT.remove_object(bucket_name, object_name)
        await _CLIENT.remove_bucket(bucket_name)


async def test_presigned_put_object_expiry(  # pylint: disable=invalid-name
        log_entry):
...
async def test_presigned_post_policy(log_entry):
    ...
    try:
        ...
        await _CLIENT.presigned_post_policy(policy)
    finally:
        await _CLIENT.remove_bucket(bucket_name)


async def test_thread_safe(log_entry):
    """Test thread safety."""
    ...
    async def get_object_and_check(index):
        try:
            local_file = "copied_file_{0}".format(index)
            await _CLIENT.fget_object(bucket_name, object_name, local_file)
    ...
    try:
        ...
        for _ in range(no_of_threads):
            thread = Thread(target=await _CLIENT.fput_object,
...
async def test_get_bucket_policy(log_entry):
    ...
    await _CLIENT.make_bucket(bucket_name)
    try:
        await _CLIENT.get_bucket_policy(bucket_name)
    except S3Error as exc:
...
async def _get_policy_actions(stat):
    """Get policy actions from stat information."""

    async def listit(value):
...
async def _validate_policy(bucket_name, policy):
    """Validate policy."""
    policy_dict = json.loads(await _CLIENT.get_bucket_policy(bucket_name))
...
async def test_get_bucket_notification(log_entry):
    ...
    await _CLIENT.make_bucket(bucket_name)
    try:
        config = await _CLIENT.get_bucket_notification(bucket_name)
        if (
...
async def test_set_bucket_policy_readonly(log_entry):
    ...
    try:
        ...
        await _CLIENT.set_bucket_policy(bucket_name, json.dumps(policy))
...
async def test_set_bucket_policy_readwrite(  # pylint: disable=invalid-name
        log_entry):
...
async def _test_remove_objects(log_entry, version_check=False):
    """Test remove_objects()."""
    ...
    try:
        ...
        errs = await _CLIENT.remove_objects(bucket_name, delete_object_list)
        for err in errs:
...
async def test_remove_objects(log_entry):
    """Test remove_objects()."""
    _test_remove_objects(log_entry)


async def test_remove_object_versions(log_entry):
    """Test remove_objects()."""
    _test_remove_objects(log_entry, version_check=True)


async def test_remove_bucket(log_entry):
    """Test remove_bucket()."""
    ...
    if _IS_AWS:
        ...
        await _CLIENT.make_bucket(bucket_name, location)
    else:
        await _CLIENT.make_bucket(bucket_name)
...
async def main():
    ...
    # Enable trace
    # await _CLIENT.trace_on(sys.stderr)
