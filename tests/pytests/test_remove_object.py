...
async def test_object_is_string():
    ...
    with pytest.raises(TypeError):
        await client.remove_object('hello', 1234)

@pytest.mark.asyncio
async def test_object_is_not_empty_string():
    client = Minio('localhost:9000')
    with pytest.raises(ValueError):
        await client.remove_object('hello', '  \t \n  ')

@pytest.mark.asyncio
async def test_remove_bucket_invalid_name():
    client = Minio('localhost:9000')
    with pytest.raises(ValueError):
        await client.remove_object('AB*CD', 'world')

@pytest.mark.asyncio
async def test_remove_object_works(mock_client_session):
    ...
    client = Minio('localhost:9000')
    await client.remove_object('hello', 'world')
